    ...
    <tr>
        <td style="padding:0 30px">
            <table align="center" cellpadding="0" cellspacing="0" border="0" width="600" style="margin:0 auto;width:600px;padding:0;border-collapse:collapse;background:white">
                ...
                <tr>
                    <td style="font-family:Arial;font-size:14px;padding:0 30px 25px;line-height:24px">
                        <hr style="border:0;border-bottom:1px solid #ececec;margin-bottom: 25px" />
                        <table align="center" cellpadding="0" cellspacing="0" border="0" width="600" style="margin:0 auto;width:600px;padding:0;border-collapse:collapse;background:white">
                            @foreach($properties as $p)
                                <tr>
                                    <td>{{ $p->name }}</td>
                                    <td>{{ $p->number }}</td>
                                    <td><a href="{{route('front.investment.property', ['investment_id' => $p->investment_id, 'floor' => $p->floor_id, 'property' => $p->id])}}">ZOBACZ</a></td>
                                </tr>
                            @endforeach
                        </table>
                    </td>
